...
print('Build model...')
model = TextAttBiRNN(embedding_matrix=embeddings, maxlen=data.shape[1], max_features=num_words,
...
flatten_layer = K.function([model.get_layer("input").input, K.learning_phase()], [model.get_layer("attention").output])
flatten_layer_vec = flatten_layer([data, 0])[0]
print(flatten_layer_vec)
with open("attbirnn.txt", "w", encoding="utf-8") as f:
